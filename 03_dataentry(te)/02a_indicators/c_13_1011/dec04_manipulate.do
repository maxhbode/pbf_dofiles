...
* Open
u "$TEMP/de03_1112(renamed)", clear
...
run "$DO/02_treatment(t)/02_dataentry(te)/c_13_1011/dec04b_valuecorrection.do"
...
fn c*, remove(c0_section_skip *comments)
foreach var in `r(varlist)' {
...
* Rename "FAKE G" (that is the sampled G)
forval var = 1/2 {
...
foreach var in `r(varlist)' {
	...
	replace b00_`suffix' = b01_`suffix' if id_phcu_ym==26051311
	replace b01_`suffix' = .d if id_phcu_ym==26051311
	replace b02_`suffix' = .d if id_phcu_ym==26051311
}
...
outsheet `r(varlist)' using "$DESKTOP/problems.csv", c replace
restore
...
qui compress
sa "$TEMP/dec04(manipulate)", replace
