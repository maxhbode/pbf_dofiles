...
foreach var in `r(varlist)' {
	...
	loc newvar = substr("`var'",1,9)
	loc varlist `varlist' `newvar'
}


reshape long `varlist', i(id) j(cat)
ren *_cat *
...
foreach var in `r(varlist)' {
	...
	loc var = substr("`var'",1,3)

	di as error "`var'_g+`var'_t"
	cap {
		g `var'_g2 = `var'_g+`var'_t
		drop `var'_g
		ren *g2 *g
	}
...
*---------------------------------------------------
compress
sa "$CLEAN/pricelist_14_070809(a04label)", replace
